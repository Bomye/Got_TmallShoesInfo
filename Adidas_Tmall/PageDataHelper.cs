...
namespace Adidas_Tmall
{
    class PageDataHelper
    {
        ...
        public List<urlInfo> GetIndexList(string url, int pg, List<urlInfo> list)
        {
            ...
            http.DefaultHeadInfo.cookie_collection.Add(uri, new System.Net.Cookie("t", "0ec33a6fbbbf7e58b630174e968c6837"));
            http.DefaultHeadInfo.cookie_collection.Add(uri, new System.Net.Cookie("_tb_token_", "irvdYdECzeDB"));
            http.DefaultHeadInfo.cookie_collection.Add(uri, new System.Net.Cookie("cookie2", "bd8ab771b1be0ee120f4a248d8e7711d"));
        ...
        public static GoodsInfo GotDetailData(urlInfo ui)
        {
            ...
            try
            {
                GetAdidasTmall.driver.Navigate().GoToUrl(detailUrl);
                LoginHint(detailUrl, GetAdidasTmall.driver.PageSource);
               
            }
            catch (Exception)
            {
                GetAdidasTmall.driver.Close();
                GetAdidasTmall.driver = new OpenQA.Selenium.Chrome.ChromeDriver(@"C:\Program Files (x86)\Google\Chrome\Application");
                GetAdidasTmall.driver.Navigate().GoToUrl(detailUrl);
            }
            ...
            hd.LoadHtml(GetAdidasTmall.driver.PageSource);
            try
            {
        ...
        static void LoginHint(string url, string pageStr)
        {
            if (pageStr.Contains("<title>上天猫，就够了</title>"))
            {
                ...
                Console.ReadKey();
                GetAdidasTmall.driver.Navigate().GoToUrl(url);
                Thread.Sleep(3000);
            }
