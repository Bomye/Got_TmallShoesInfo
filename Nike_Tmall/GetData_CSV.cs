...
namespace Nike_Tmall
{
    class GetData_CSV
    {
        public static void GotResultCsv(string excelPath, string tableName, string excelName)
        {
            ...
            List<Tmall_Detail_Nike> orm_List = ORMHelper.GetModel<Tmall_Detail_Nike>("where  LastUpdate > '2017-04-10 12:00:00'");
            Dictionary<ulong, Tmall_Detail_Nike> orm_Dic = new Dictionary<ulong, Tmall_Detail_Nike>();
            foreach (var ol in orm_List)
            {
                if (!orm_Dic.ContainsKey(ol.Id)) orm_Dic.Add(ol.Id, ol);
            }
            var dic_Xiajia = new Dictionary<ulong, string[]>();
            var dic_Shangjia = new Dictionary<ulong, Tmall_Detail_Nike>();
            var dic_SjNo = new Dictionary<ulong, Tmall_Detail_Nike>();//上期没有数据但是以前有的
            ...
            using (System.IO.StreamWriter sw = new System.IO.StreamWriter("result_Nike.csv", false, Encoding.UTF8))
            {
                ...
                {
                    if (IsNew((long)dsj.Key, dsj.Value.LastUpdate, "tmall_detail_nike"))
                    {
